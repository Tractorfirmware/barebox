...
static void i2c_dw_enable(struct dw_i2c_dev *dw, bool enable)
{
	...
	do {
		...
		/*
		 * Define sleep interval 10 times the signaling period
		 * for the highest I2C transfer supported by the
		 * controller (for 400KHz this is 25us) as per
		 * "Cyclone V Hard Processor System Technical
		 * Reference Manual"
		 */
		udelay(250);
	} while (timeout--);
				...
				uint32_t mask, uint32_t value, uint64_t timeout)
{
	...
	pr_err("Timeout waiting for 0x%08x @ 0x%08x\n",
	       mask, offset);
...
static int i2c_dw_wait_for_no_activity(struct dw_i2c_dev *dw)
{
	const uint32_t mask  = DW_IC_STATUS_MST_ACTIVITY;
	const uint32_t value = DW_IC_STATUS_MST_ACTIVITY;

	int ret;

	ret = i2c_dw_wait_for_bits(dw, DW_IC_STATUS, mask, value,
				   DW_TIMEOUT_TX);

	return (ret < 0) ? ret : i2c_dw_wait_for_idle(dw);
}
...
static int i2c_dw_finish_xfer(struct dw_i2c_dev *dw, bool data_only)
{
	int ret;
	uint32_t ic_int_stat;
	...
	if (data_only) {
		...
		/*
		  If it is not just a data-only transfer it means that
		  we should expect the controller to signal STOP
		  condition on the bus, so we are going to wait for it
		  first.
		 */
		...
		/*
		  Now that we now that the stop condition has been
		  signaled we need to wait for controller to go into
		  IDLE state to make sure all of the possible error
		  conditions on the bus have been flagged in
		  apporpriate status registers
		 */
...
static int i2c_dw_set_address(struct dw_i2c_dev *dw, uint8_t address)
{
	int ret;
	uint32_t ic_tar;
	/* We have to wait for idle in order to be
	 * able to set the address dynamically */
	ret = i2c_dw_wait_for_idle(dw);
	if (ret < 0)
		return ret;
		       ...
		       struct i2c_msg *msgs, int num)
{
	...
	for (i = 0; i < num; i++) {
		const bool data_only = msgs[i].flags & I2C_M_DATA_ONLY;
		const bool read = msgs[i].flags & I2C_M_RD;

		pr_info("\n [%d] len: %d, addr: %x, flags: %x buf[0]: %x\n", i, msgs[i].len, msgs[i].addr, msgs[i].flags, msgs[i].buf[0]);
		...
		if (read)
			ret = i2c_dw_read(dw, &msgs[i], data_only);
		else
			ret = i2c_dw_write(dw, &msgs[i], data_only);

		if (ret < 0)
			break;

		ret = i2c_dw_finish_xfer(dw, data_only);
		if (ret < 0)
			break;
	}

	if (ret == -EIO) {
		const uint32_t ic_tx_abrt_source = readl(dw->base + DW_IC_TX_ABRT_SOURCE);

		dev_err(&dw->adapter.dev, "abort source: %x\n", ic_tx_abrt_source);
...
static int i2c_dw_probe(struct device_d *pdev)
{
	...
	pr_info("Initializing I2C driver\n");
	...
	/*
	   ...
	   wouldn't clear. We don't care about the contents of that
	   aregister so we just set it all to zero.
	 */
...
fail:
	...
	ic_con = readl(dw->base + DW_IC_CON);
	pr_info("Doen Initializing I2C driver con=%x ret=%d\n", ic_con, ret);
